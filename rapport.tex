...
\textit{Insérez ici votre diagramme. Assurez-vous d'avoir le fichier image dans le même dossier.}

\begin{figure}[H]
    \centering
    % Décommentez la ligne ci-dessous une fois que vous avez l'image :
    % \includegraphics[width=0.9\textwidth]{diagramme_ea.png} 
    \caption{Diagramme Entité-Association du projet}
    \label{fig:mcd}
\end{figure}
...
Traduction du modèle E-A vers le relationnel (les clés primaires sont soulignées, les clés étrangères précédées d'un \#) :

\begin{itemize}
    \item \textbf{Restaurant} (\underline{restaurant\_id}, nom, adresse, coordonnees\_gps)
    \item \textbf{Client} (\underline{client\_id}, nom, email, adresse)
    \item \textbf{Commande} (\underline{commande\_id}, date\_commande, \#client\_id, \#restaurant\_id)
    \item \textbf{Item} (\underline{item\_id}, nom, prix, \#restaurant\_id, \#categorie\_id)
\end{itemize}
...
\textbf{Extrait du script de création (create.sql) :}
\begin{lstlisting}[language=SQL]
CREATE TABLE restaurants (
    restaurant_id SERIAL PRIMARY KEY,
    nom VARCHAR(255) NOT NULL,
    adresse TEXT NOT NULL,
    -- Utilisation de PostGIS pour la géolocalisation
...
\begin{enumerate}
    \item Importer la structure via \texttt{create.sql}.
    \item Peupler la base via \texttt{insert.sql}.
    \item Accéder à \texttt{index.php}.
    \item Se connecter (ex: \texttt{alice.dupont@email.fr}).
\end{enumerate}

\subsection{Description des interfaces}

\subsubsection{Tunnel de commande (Formules)}
Le client configure sa formule étape par étape (Entrée $\rightarrow$ Plat $\rightarrow$ Dessert). Les choix sont stockés en \texttt{\$\_SESSION} et persistés en base uniquement à la fin via une transaction SQL atomique.
