...
Le site permet en plus d’informer les utilisateurs sur la composition nutritionnelle des produits, notamment leur teneur énergétique et en protéines. Les clients peuvent suivre l’avancement de leur commande à chaque étape, depuis la sélection des plats jusqu’à la réception, et consulter l’historique de leurs commandes passées. \\

Enfin, une interface restaurateur a été ajoutée, permettant au restaurant de gérer ses plats, formules et horaires d'ouverture ainsi que de voir quelques statistiques.
...
\begin{itemize}
    ...
    \item \textbf{RG03 :} Un item appartient à un seul restaurant.
...
\begin{itemize}
    ...
    \item \textbf{RG07 :} Via ses commandes, un client qui a un compte lié (au restaurant) cumule des points de fidélité indépendamment pour chaque restaurant. Ce compte est créé automatiquement lors de la première commande d'un utilisateur qui n'est pas un invité.
    ...
    \item \textbf{RG09 :} Si le client a un compte lié au restaurant, il peut déposer un commentaire contenant une note, visible par les autres clients.
\end{itemize}

\paragraph{Commandes}
\begin{itemize}
    \item \textbf{RG10 :} Une commande est passée par un client auprès d'un unique restaurant (pas de commande multi-restaurants).
    \item \textbf{RG11 :} Un client ne peut avoir qu'une seule commande "en cours" (non achevée) par restaurant à un instant T. Il peut passer plusieurs commandes en cours à plusieurs restaurants différents.
    ...
    \item \textbf{RG14 :} Le prix total d'une commande est calculé et mis à jour automatiquement à chaque ajout, modification ou suppression d'item ou de formule.
    \item \textbf{RG15 :} La suppression d'une commande entraîne la suppression automatique de tout son contenu (items et formules).
    \item \textbf{RG16 :} Le client devra renseigner une heure de livraison ne dépassant pas 5h après l'heure de la commande. Il pourra aussi choisir une option de livraison "au plus vite".
\end{itemize}

\paragraph{Formules (ou Menu)}
\begin{itemize}
    \item \textbf{RG17 :} Une formule est définie par un prix fixe et une structure de catégories (ex: 1 Entrée + 1 Plat).
    \item \textbf{RG18 :} Une formule peut avoir des conditions de validité (jours de la semaine, horaires, midi/soir).
    \item \textbf{RG19 :} Lors de l'achat d'une formule, les items choisis par le client sont liés à une instance spécifique de cette formule dans la commande, afin de distinguer les items "au menu" des items "à la carte".
\end{itemize}

\paragraph{Géolocalisation}
\begin{itemize}
    \item \textbf{RG20 :} La recherche par proximité affiche uniquement les restaurants situés dans un rayon défini (par défaut 2km) autour de la position de l'utilisateur.
    \item \textbf{RG21 :} La distance est calculée "à vol d'oiseau" sur la sphère terrestre.
\end{itemize}
...
Nous voulions passer la composition de la formule via son titre, en effectuant un traitement de texte, mais cette approche ne respectait pas la 1NF.

\paragraph{Correction apportée :}
Pour réparer cette erreur de conception, nous avons ajouté l'association \textbf{\texttt{composer\_formule}} (visible sur le schéma final) reliant l'entité \texttt{Formules} à l'entité \texttt{Categories\_items}.
...
Après avoir cherché des renseignements, nous avons décidé de suivre l'architecture Modèle-Vue-Contrôleur pour notre projet. Cette structuration nous a permis de délimiter les responsabilités des fichiers; ce qui est important car le projet traite des règles de gestion complexes mêlant SQL, PHP et HTML/CSS (interface web interactive). De plus, cela nous a aidé à apporter de la clarté à un projet qui s'annonçait de taille. \\

Concrètement, l'application tourne autour de trois composants distincts : 
\begin{itemize} 
    \item \textbf{Le Modèle (M) :} C'est la couche d'accès aux données stockées dans la base via des requêtes SQL. Des modèles d'objet représentant généralement des entités du projet sont stockés dans \texttt{model}, elle encapsule les requêtes SQL qui leur sont liées dans des fonctions paramétrées et réutilisables. On peut y traiter des requêtes complexes (comme des transactions atomiques) pour n'envoyer que l'information utile aux fichiers PHP.
    
    \item \textbf{La Vue (V) :} Dans le dossier \texttt{views}, les vues gèrent quasi-exclusivement la présentation. Elles reçoivent des données brutes obtenues via le contrôleur (sous forme de tableaux PHP) et les transforment en code HTML couplé avec du CSS afin d'avoir un rendu propre et intelligible pour l'utilisateur (cartes de restaurants, tableaux de menus, formulaires). Elles contiennent le moins possible de logique métier (seulement des boucles de traitement pour l'affichage et quelques fonctions Javascript pour des affichages spéciaux) et aucune requête SQL, ce qui facilite la maintenance du design et l'intégration du CSS. 
    
    \item \textbf{Le Contrôleur (C) :} C'est ici qu'on implémente principalement la logique métier, il fait la liaison entre les vues et les modèles. Il vérifie certaines conditions (par exemple la nature de l'utilisateur), ou des informations passées via l'URL, la session ou des méthodes POST. Puis il traite et regroupe les résultats des fonctions des modèles appelées afin qu'ils soient transformés en data utilisable par la vue.
\end{itemize} 

\textbf{Justification et Avantages :} Le choix de MVC offre plusieurs avantages majeurs pour ce projet : 
\begin{enumerate} 
    \item \textbf{Séparation des responsabilités :} Le modèle permet de séparer les responsabilités, ainsi si on veut changer la structure de la base de données, on peut potentiellement ne modifier que les requêtes et les fonctions dans les modèles sans toucher à la logique métier et à l'affichage.
    
    \item \textbf{Sécurité et Robustesse :} Grâce aux vérifications du contrôleur, les entrées de l'utilisateur sont séparées de la couche de données avant d'être validées, ce qui réduit le risque d'injections ou d'erreur de logique.
    
    \item \textbf{Travail collaboratif :} La structure a pu permettre de mettre en place un parallélisme dans le développement, permettant aux membres du projet de travailler simultanément sur des parties distinctes du code, réduisant les risques de fusions conflictuelles.
\end{enumerate}

\subsection{Implémentation des opérations}

Nous avons choisi de déporter une partie de la logique métier directement dans la base de données via des procédures stockées PL/pgSQL. Notamment dans les cas où une réactivité rapide de l'affichage était voulue. Cela garantit la rapidité d'exécution et l'intégrité des données.
...
FOR EACH ROWde liste déroulante dynamique
...
\begin{enumerate}
    \item \textbf{À la carte :} Ajout direct d'items au panier. Si l'item possède des accompagnements qui lui sont directement lié (sauces, accompagnements), un système permet de proposer l'ajout de ces compléments uniquement si l'item principal est choisi.
...
\end{enumerate}
Au fur et à mesure que la commande est créée, un petit aperçu de celle-ci est visible sur la droite de l'écran. Cela permet à l'utilisateur de savoir ce qu'il a commandé.
L'utilisateur peut renseigner l'heure de livraison de sa commande.
...
Les commandes sont validées (On imagine qu'un paiement peut être mis en place à ce moment-là) dans la section \textbf{panier}.
Puis, le client peut valider la réception dans la section \textbf{suivi}, il atteste avoir bien reçu la commande.
Enfin, les informations de la commande sont passées dans un \textbf{historique} que le client peut consulter à tout moment.
...
Le restaurateur dispose d'outils lui permettant la création/insertion ainsi que certaines modifications et suppressions :
\begin{itemize}
    \item \textbf{Création de Plats :} Ajout de nouveaux items avec définition du prix, de la catégorie et association possible avec des compléments existants. On peut gérer l'ajout d'ingrédients composant le plat (quantité, informations nutritionnelles), ainsi que la disponibilité du plat (sa visibilité pour le client).
    \item \textbf{Création de Formules :} Interface permettant de définir un prix global, de composer la structure du menu (ex: choisir les catégories "Entrée" et "Plat") et de définir des conditions de validité (jours, horaires). On peut aussi supprimer les formules. La modification des attributs de formules n'a pas été mis en place.
...
\begin{itemize}
    \item \textbf{Serveur :} Nous utilisons le serveur de l'école.
    \item \textbf{Base de données :} phpMyAdmin 
    \item \textbf{Langage :} PHP 7.4+, PostgreSQL 13+
...
\begin{enumerate}
    ...
    \item Accéder au site via l'URL suivante https://zzz.tabeille001.bordeaux-inp.fr/src/index.php en s'assurant d'être connecté sur le réseau de Bordeaux INP
    \item Enfin, se connecter/créer un compte en utilisant un email ou utiliser le mode invité.
    \item Pour détruire la base, utiliser le fichier \texttt{drop.sql}.
\end{enumerate}
